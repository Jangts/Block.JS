...
return {
	...
	var check = function (property, value) {
		switch (property.base) {
			...
			case 'any': returncheckAny(property, value);
		}
		return false;
	};
	...
	var checkTime = function (property, value) {
		if(_.util.bool.isStr(value))  {
			switch (property.type) {
				...
				case 'hourminute': return/^\s*\d{1,2}\:\d{1,2}\s*$/.test(value);
				default: return/^\s*\d{4}\-\d{1,2}\-\d{1,2}\s\d{1,2}\:\d{1,2}\:\d{1,2}\s*$/.test(value);
			};
		}
		return false;
	};
	var checkNumber = function (property, value) {
		switch (property.type) {
			...
			default: return _.util.bool.isNumeric(value) && checkLength(property.length, value.toString()) && checkRange(property.range, value);
		};
	};
