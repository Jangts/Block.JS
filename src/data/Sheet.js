...
tangram.block([
	/* @posi0 */'$_/arr/',
	/* @posi1 */'$_/obj/',
	/* @posi2 */'$_/data/Storage'
], function (pandora, global, imports, undefined) {
	var _ = pandora;
	var doc = global.document;
	var console = global.console;
	var location = global.location;
	var Storage = _.data.Storage;
	var queryJSON = _.data.queryJSON;
	var infs = {};
	var tables = {};
	...
	var update = function (tablename) {
		...
		var json = global.JSON.stringify(tables[tablename])
		...
		for (i;i < parts;i++) {
			...
			new new Storage(storageName).set('data', json.substr(partlen * i, partlen));
		}
	...
	pandora.declareClass('data.Sheet', {
			...
			if (namingExpr.test(tablename)) {
				...
				if (!tables[tablename]) {
					...
					pandora.each(mateinfs.storages, function (i, storageName) {
						json += new new Storage(storageName).get('data')
					}, this);
		...
		drop: function (fieldname) {
			...
			pandora.each(mateinfs.storages, function (i, storageName) {
				new new Storage(storageName).clear(true);
			}, this);
