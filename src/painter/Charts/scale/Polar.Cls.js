...
block([
    ...
    declare('painter.Charts.scale.Polar', _.painter.Charts.Component, {
        ...
        getValues: function() {
            ...
            if (this.status === 6) {
                ...
                _.each(this.instance.series.radar, function(i, chart) {
                    max.push(_.util.arr.max(chart.data));
                    min.push(_.util.arr.min(chart.data));
                ...
                this.radiusAxisMax = _.util.arr.max(max);
                this.radiusAxisMin = _.util.arr.min(min);
            } else {
                _.each(this.instance.series.line, function(i, chart) {
                    ...
                    this.radiusValues = _.util.arr.merge(this.radiusValues, data);
                }, this);
                _.each(this.instance.series.scatter, function(i, chart) {
                    ...
                    switch (this.status) {
                        case 0:
                            this.radiusValues = _.util.arr.merge(this.radiusValues, radiusData);
                            break;
                        case 4:
                            this.angleValues = _.util.arr.merge(this.angleValues, angleData);
                            this.radiusValues = _.util.arr.merge(this.radiusValues, radiusData);
                            break;
                    }
                }, this);
                //this.radiusAxisMax = _.util.arr.max(this.radiusValues);
                //this.radiusAxisMin = _.util.arr.min(this.radiusValues);
                this.radiusAxisMax = _.util.arr.max(this.radiusValues);
                this.radiusAxisMin = _.util.arr.min(this.radiusValues);
        ...
        calculateAngleRange: function() {
            if (this.status === 0 || this.status === 5 || this.status === 6) {
                ...
                if ((typeof(this.angleAxisMax) === 'number') && (typeof(this.angleAxisMin) === 'number')) {
                    ...
                    var valuesArray = [this.angleAxisMax, _.util.arr.min(this.angleValues)];
                } else if (typeof(this.yAxisMin) === 'number') {
                    var valuesArray = [this.angleAxisMin, _.util.arr.max(this.angleValues)];
                } else {
